...
void ASimHUD::BeginPlay()
{
    ...
    APlayerController* current_player_controller = Cast<APlayerController>(this->GetOwner());
    if (!current_player_controller) {
        // 如果没有找到当前的玩家控制器，则直接返回
        return;
    }
    // 获取当前的玩家控制器索引
    int32 player_index = UGameplayStatics::GetPlayerControllerID(current_player_controller);
    // 只在第一个玩家（Player0）处进行初始化
    if (player_index != 0)
        return;
