...
void ASimModeBase::startApiServer()
{
    if (getSettings().enable_rpc) {
        ...
        easyTrack_rpc_server_ = std::make_unique<UEeasyTrackRpcServer>();
        //woods
#endif
        try {
            api_server_->start(false, spawned_actors_.Num() + 4);
            easyTrack_rpc_server_->startServer();
            UAirBlueprintLib::LogMessage(TEXT("easyTrack_rpc_server_->startServer()"), " ",
                                         LogDebugLevel::Informational);
        }
...
void ASimModeBase::stopApiServer()
{
    ...
    if (easyTrack_rpc_server_ != nullptr) {
       easyTrack_rpc_server_->stopServer();
       easyTrack_rpc_server_.reset(nullptr);
    }
